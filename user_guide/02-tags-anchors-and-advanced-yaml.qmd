...
### Mapping-key collisions

Duplicate mapping keys are unsupported. Do not rely on which value is
retained when a YAML mapping repeats a key.

YAML keys that are distinct can also compare equal after conversion to
Python. For example, Python considers `True` equal to `1`, so the second
value overwrites the first when these keys are inserted into a `dict`:

```{python}
parsed = parse_yaml("1: integer\ntrue: boolean\n")
key = next(iter(parsed))

assert type(key) is int
assert parsed == {1: "boolean"}
```
...
output starts each document with `---` and ends with a newline. It does
not add optional `...` markers. For single documents, `write_yaml()`
wraps the body with `---` and adds a final newline, while `format_yaml()`
...
`Yaml` stores simple local tag names in normalized `!name` form. To
inspect the normalized form, construct a temporary `Yaml` and read its
`.tag`:

```{python}
assert Yaml(None, "gizmo").tag == "!gizmo"
assert Yaml(None, "!<gizmo>").tag == "!gizmo"
```
...
item: !<gizmo> foo
""")
parsed["item"].tag  # verbatim tag, not expanded
```
